...
{    
    ...
    if(llJsonGetValue(JSON, ["type"]) == "playing")
    {
        ...
        if(priorityTag < llGetUnixTime())
        {
            ...
            /|/ calc percentage
            float divisor = (float)llJsonGetValue(JSON, ["totalTime"]);
            if (divisor <= 0.0)
                divisor = 1000.0;
            float tmp_percent = (float)llJsonGetValue(JSON, ["time"]) / divisor;
    ...
    if(llJsonGetValue(JSON, ["type"]) == "loading")
    {
        ...
        /|/ build display
        float divisor = (float)llJsonGetValue(JSON, [ "end" ]);
        if (divisor <= 0.0)
            divisor = 1000.0;
        float tmp_percent = (float)llJsonGetValue(JSON, ["current"]) / divisor;
...
//last_compiled 03/15/2024 15:49:15
//mono
