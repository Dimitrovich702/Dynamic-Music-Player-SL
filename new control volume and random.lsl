...
string VOL_U_MSG = "ᴠᴏʟ +";
string VOL_D_MSG = "ᴠᴏʟ -";
string RANDO_MSG = "ʀᴀɴᴅᴏ";
...
curSongs()
{
    ...
    if(curSongEnd < (totalSongs-1))
        but += [STOP_MSG, NEXT_MSG, VOL_U_MSG, RANDO_MSG, VOL_D_MSG];
    else
        but += [STOP_MSG, "🚫", VOL_U_MSG, RANDO_MSG, VOL_D_MSG];
...
LoadSong()
{
      integer rando = (integer)llFrand(llGetListLength(rewinds));
  string rewindz = llList2String(rewinds, rando);
  llMessageLinked(2, 0, rewindz, " ");
    Debug("LoadSong");
...
StopSong()
{
      integer rando = (integer)llFrand(llGetListLength(stops));
  string stopz = llList2String(stops, rando);
  llMessageLinked(2, 0, stopz, " ");
    Debug("StopSong");
